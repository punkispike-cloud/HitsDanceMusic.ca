/* Pose la radio courante (radio_id) sur le contexte.
   ...
   - adminTenant  : admin authentifié → radio de l'utilisateur (non-owner) ;
     l'owner (En Ondes) choisit la radio (header X-Radio-Id ou ?radio=) ou
     retombe sur l'unique radio du parc. */
...
export const adminTenant: MiddlewareHandler<AppBindings> = async (c, next) => {
  ...
  if (user.role !== "owner") {
    // Admin / animateur / lecteur : strictement leur radio.
    c.set("radioId", user.radioId ?? (await soleRadioId()));
    return next();
  }
  // Owner : sélection explicite d'une radio, sinon l'unique radio du parc.

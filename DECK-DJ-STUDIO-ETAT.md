...
| ✅ **Analyseur BPM** | **FAIT** : tap tempo + ré-analyse (`reanalyzeBpm`) + édition manuelle par deck (studio), **indicateur de battement** (pulse au tempo), et **analyse auto au téléversement** dans `/pistes` (estime + PATCH `bpm` si vide, via `OfflineAudioContext` + `estimateBpm`). |

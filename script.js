...
function applyTheme(mode) {
  ...
  // On stocke l'intention de l'utilisateur (mode) ET le rendu effectif (resolved)
...
// Position par défaut : Québec (Vieux-Québec)
const WEATHER_DEFAULT = { lat: 46.8139, lon: -71.2080, name: "Québec" };

// Cache léger (24 h) pour la position et le nom de ville géolocalisés
...
// Géolocalisation navigateur (silencieuse, refus = on ignore)
function getUserPosition() {
  return new Promise((resolve) => {
    if (!("geolocation" in navigator) || !window.isSecureContext) return resolve(null);
    // Si l'utilisateur a déjà refusé (Permissions API), on ne re-prompt pas.
...
// Reverse-geocode via Open-Meteo (gratuit, sans clé). Échec silencieux.
...
async function loadWeather() {
  ...
  // 1) Choix de la position : cache > géoloc > défaut Québec
  ...
  // 2) Récupère la météo pour la position retenue
  try {
